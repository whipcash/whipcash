...
#if defined(NDEBUG)
# error "Whipcash cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5f0db2dfb8ad0abd1fd21172465c20907e6eec11bf95d8c0ef7bcda65cd63ab7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Whipcash: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Whipcash Signed Message:\n";
...
{
    // Whipcash: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Whipcash: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 7 * 24 * 60 * 60; // Whipcash: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Whipcash: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Whipcash: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Whipcash: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5f0db2dfb8ad0abd1fd21172465c20907e6eec11bf95d8c0ef7bcda65cd63ab7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC News 13 May 2015 North Korea Defence Chief Hyon Yong-chol 'executed'";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1431569751;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1077071026;

        if (fTestNet)
        {
            block.nTime    = 1317798646;
            block.nNonce   = 8298497823;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x7ea4a16305f75639537e7af73cd87f9b6e57dd66bc1b9992d584f47d50b00a0f"));
        block.print();
/*            if (hash != hashGenesisBlock)
    {
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Whipcash: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WhipcashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WhipcashMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WhipcashMiner(CWallet *pwallet)
{
    printf("WhipcashMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("whipcash-miner");
    ...
    try { loop {
        ...
        printf("Running WhipcashMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WhipcashMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WhipcashMiner, pwallet));
}
